...
- **`src/index.js`** — the Worker entry. `scheduled()` runs on every Cron Trigger fire and runs every integration in `src/integrations.js` (immediate check, or recap on the once-daily trigger) — see "Why LAFC didn't need its own Cron Triggers" below. `fetch()` exposes a `/test-notify` route (with an optional `?target=<integration id>|ops`) for manually verifying the ntfy path. Neither function knows anything sport-specific — adding a third integration doesn't touch this file.
- **`src/jobs.js`** — two generic, integration-agnostic job runners, each taking an integration config (see `src/integrations.js`) as a parameter:
  - `checkAndNotify(env, deps, integration)` — look for a finished qualifying game, publish once immediately if unflagged.
  - `sendMorningRecap(env, deps, integration)` — bundle any unflagged qualifying games into one recap message the next morning.
  - `alertOps()` — if an integration's data source throws and the integration opts in (`opsLabel`, currently just LAFC/ESPN), pages `NTFY_OPS_TOPIC` once and then stays quiet on that failure kind for 6h (a KV flag), instead of re-alerting every 15 minutes or failing silently.
- **`src/integrations.js`** — the registry: one config object per sport/promo (Dodgers, LAFC), each describing how to fetch its games, which ones qualify, how to key/dedupe them in KV, which ntfy topics to publish to, and the message templates. This is the file you edit to add a new integration or tweak an existing one's behavior — see "Adding a new integration" under Customizing.
...
- **`src/mls.js`** — polls ESPN's unofficial soccer API for LAFC's games in the last ~24h, and for each finished **home** game, inspects the play-by-play (`keyEvents`) to determine whether LAFC scored the match's first (non-shootout) goal and whether it fell in the first half. Unlike `mlb.js`, failures are deliberately not left to crash the run: a bad HTTP response or an unrecognized JSON shape is thrown as an `Error` with `.kind` set to `"fetch"` or `"schema"` respectively, so `jobs.js` can route it to an ops alert instead of going dark. See "Why LAFC needed error-shape handling that MLB didn't" below. It also accepts an optional `skip(eventId)` callback (supplied by `jobs.js`) so it can avoid an extra per-event summary fetch for games already handled — see "Why the LAFC check avoids re-fetching finished games" below.
- **`src/ntfy.js`** — publishes a message to one or more ntfy topics via HTTP POST. `topics` is a required, explicit param on every call — nothing defaults silently. `dodgersTopics(env)`/`lafcTopics(env)` are the per-integration topic lists that `src/integrations.js` points at, so a new integration adds its own `xTopics(env)` here rather than teaching `publish()` about another special case.
- **`test.js`** — `npm test` runs a mocked pass with fake KV storage and no real network calls, driving the generic `checkAndNotify`/`sendMorningRecap` runners against the real Dodgers and LAFC configs from `src/integrations.js` (with `getGames` swapped for a mock). Covers: home win publishes, away win/home loss don't, doubleheaders publish separately, re-runs don't double-publish (both integrations), plus the ops-alert path (a fetch failure and a schema failure each alert once and dedupe on repeat).
- **`wrangler.toml`** — Worker config: the Cron Trigger schedule, non-secret vars (including the ntfy server URL and every integration's topics), and the KV namespace binding.
...
### Why the LAFC check avoids re-fetching finished games

`src/mlb.js` gets everything it needs (score, venue) from one schedule call. `src/mls.js` needs a second, per-game ESPN request (`/summary?event=...`) to inspect play-by-play, since the schedule alone doesn't say who scored first. A finished home game sits in the 24h lookback window across roughly forty 15-minute check ticks, so without care that's forty repeated summary fetches for a game whose outcome is already known and already published — needless load, and needless exposure to an unsupported API's rate limits. `jobs.js` passes `getGames` a `skip(eventId)` check (backed by the same KV flag it's about to consult anyway); `mls.js` calls it before the summary fetch and moves on if the game's already been handled for that job. `mlb.js` ignores the option entirely — it has no equivalent per-game cost to save.
...
- **Different team (LAFC):** change `ESPN_LAFC_TEAM_ID` in `wrangler.toml` to another MLS team's ESPN id, and rewrite the "scored first in the first half" condition in `scoredFirstInFirstHalf()` (`src/mls.js`) if the new promo's trigger is different (e.g. plain "won at home" — the `lafc` entry's `filterHits` in `src/integrations.js` would then check `g.dodgersWon`-style logic instead of `g.scoredFirstInFirstHalf`).
- **Away wins too:** in `src/integrations.js`, change the `dodgers` entry's `filterHits` from `g.isHomeGame && g.dodgersWon` to just `g.dodgersWon`.
- **Check frequency/window:** edit the `crons` array in `wrangler.toml` (standard cron syntax, UTC only) — every integration shares it.
- **Message wording:** edit `buildMessage`/`buildSummary`/`buildRecapText` on the relevant entry in `src/integrations.js`.
- **Ops-alert dedup window:** edit `OPS_ALERT_TTL_SECONDS` in `src/jobs.js` (default 6h).
- **Adding a new integration (a third sport/promo):** `src/jobs.js` and `src/index.js` don't need to change. You need: (1) a fetcher module shaped like `src/mlb.js`/`src/mls.js` — a `getGames(env, { skip })` that returns finished games in the lookback window; (2) a `xTopics(env)` function in `src/ntfy.js`; (3) one new entry in the `INTEGRATIONS` array in `src/integrations.js` (`id`, `getGames`, `filterHits`, `idOf`, `checkKeyPrefix`/`recapKeyPrefix`, `topics`, titles, and the message-building functions — copy an existing entry as a template); (4) its topic vars (and any API config) in `wrangler.toml`. `/test-notify?target=<id>` picks it up automatically once `id` matches the registry entry.
